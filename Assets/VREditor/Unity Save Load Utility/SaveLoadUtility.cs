...
public class SaveLoadUtility : MonoBehaviour {
	...
	void Start () {
        ...
        ObjectIdentifier[] prefabs_oi = Resources.LoadAll<ObjectIdentifier>("VREditorPrefabs");
	...
	public void LoadGame(string saveGameName) {
		...
		foreach(SceneObject loadedObject in loadedGame.sceneObjects) {
			...
			if(go != null) {
				...
				SaveLoad.objectIdentifierDict.Add(oi.id, oi);
                if (StateManager.Instance.stageObject != null) go.transform.SetParent(StateManager.Instance.stageObject.transform);
			}
		}
	...
	public void ClearScene() {
		...
		foreach (object o in obj) {
            ...
            if (StateManager.Instance.stageObject != null)
            {
                if (go.transform.parent != null)
                {
                    if (go.transform.parent.name == StateManager.Instance.stageObject.transform.name)
                    {
